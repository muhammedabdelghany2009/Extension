...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ميزة الاستماع للأيقونة السفلية (أقصى الجنوب): عندما يضغط عليها المستخدم تفتح كارت الإضافة فوراً
...
async function executeDirectDynamicCheck() {
    ...
    currentVideoId = urlParams.get("v") || "custom_id";
...
// تعديل ميزة المحفوظات: تعرض اللوجو، وعند النقر يفتح العلامات بداخل القائمة بدلاً من فتح رابط جديد
    ...
    chrome.storage.local.get(null, (items) => {
        ...
        keys.forEach(key => {
            ...
            if (videoData && videoData.title) {
                ...
                // الهيكل الشيك الجديد: لوجو القناة الافتراضي الأحمر الشيك لليوتيوب، ثم العنوان
                li.innerHTML = `
          <div class="video-row-header" style="display:flex; justify-content:space-between; align-items:center; width:100%;">
            <div style="display:flex; align-items:center; gap:8px;">
              <span class="channel-logo-badge" style="background:#ff0000; color:white; border-radius:50%; width:24px; height:24px; display:inline-flex; align-items:center; justify-content:center; font-size:10px; font-weight:bold;">YT</span>
              <span style="font-weight:600; font-size:12px; color:#1e293b; max-width:200px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${videoData.title}</span>
            </div>
            <span class="badge" style="background:#e8f5e9; color:#2e7d32;">${videoData.markers.length} علامات</span>
          </div>
          <!-- منطقة مخفية لعرض العلامات التابعة لهذا الفيديو عند الضغط عليه -->
          <div class="sub-markers-list hidden" style="margin-top:10px; padding-top:8px; border-top:1px dashed #e2e8f0;"></div>
        `;

                // ميزة الضغط: تفتح العلامات التابعة له وتستعرضها أولاً بداخل القائمة
                li.addEventListener('click', (e) => {
                    const subList = li.querySelector('.sub-markers-list');

                    if (subList.classList.contains('hidden')) {
                        subList.classList.remove('hidden');
                        subList.innerHTML = ''; // تنظيف

                        videoData.markers.forEach(marker => {
                            const div = document.createElement('div');
                            div.style.cssText = 'display:flex; justify-content:space-between; padding:6px; background:#f8fafc; margin-bottom:4px; border-radius:6px; font-size:11px; cursor:pointer;';
                            div.innerHTML = `<span>📌 ${marker.title}</span> <span style="color:#3b82f6; font-weight:bold;">${marker.time}</span>`;

                            // لو ضغط على العلامة توديه ليها في الفيديو وتنقله فوراً
                            div.addEventListener('click', async (event) => {
                                event.stopPropagation(); // منع تداخل الضغطات
                                const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });

                                // الانتقال المباشر للوقت
                                chrome.tabs.sendMessage(activeTab.id, { action: "SEEK_TO", seconds: marker.seconds });
                            });
...
function exportStorageToJson() {
    chrome.storage.local.get(null, (data) => {
        const jsonString = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(data, null, 2)); const a = document.createElement('a');
        a.setAttribute("href", jsonString); a.setAttribute("download", "YT_Bookmarks_Database.json"); document.body.appendChild(a); a.click(); a.remove();
    });
...
async function startLoopSequence() {
    ...
    const to = document.getElementById('loopTo').value; const count = document.getElementById('loopCount').value; if (!from || !to) return alert("فضلاً أدخل نطاق الوقت!");
    const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true });
