...
async function executeDirectDynamicCheck() {
    ...
    // جلب التبويب الحالي النشط
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    const currentTab = tabs[0];

    // قراءة عنوان الصفحة والرابط بأمان تام
    ...
    // 1. فحص لو خارج موقع يوتيوب بالكامل
    ...
    // 2. فحص قاطع: لو داخل يوتيوب ولكن في الصفحة الرئيسية أو البحث أو القنوات (وليس فيديو شغال)
    // يوتيوب يضع كلمة "YouTube" فقط في العنوان إذا كنت بالرئيسية، بينما الفيديوهات تحمل دائماً اسم الفيديو وصانع المحتوى
    ...
    if (isMainOrSearchPage) {
        ...
        errorBtn.onclick = (e) => {
            e.preventDefault();
            window.close(); // إغلاق الإضافة فوراً ليختار الفيديو براحته
        };
    }
    // 3. مستخدم داخل فيديو حقيقي بكافة صيغه المتوقعة
    else {
        ...
        // إنشاء معرف فريد مبني على العنوان لحفظ العلامات بثبات
