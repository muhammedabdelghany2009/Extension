...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // الحل القاطع: طلب الرابط عبر الباكجراوند لتفادي حظر كروم للرابط
    chrome.runtime.sendMessage({ action: "FORCE_GET_ACTIVE_TAB" }, (response) => {
        if (response && response.tab) {
            checkCurrentLocationAndValidate(response.tab);
        } else {
            // حل احتياطي في حال فشل الجلب
            fallbackTabCheck();
        }
    });
});

// دالة الفحص الذكي والمضمون لموقع المستخدم
function checkCurrentLocationAndValidate(currentTab) {
    ...
    if (!currentTab || !currentTab.url) {
        showCustomError("يرجى فتح يوتيوب! 🎬", "لم نتمكن من تحديد الرابط الحالي، يرجى الانتقال إلى منصة يوتيوب لبدء استخدام الإضافة.", "الانتقال إلى يوتيوب 🚀", "https://youtube.com");
        return;
    }

    const urlStr = currentTab.url;

    // الحالة 1: المستخدم داخل فيديو يوتيوب فعلي (مضمونة الآن)
    ...
    // الحالة 2: المستخدم داخل موقع يوتيوب ولكن في أي مكان آخر غير الفيديو
        ...
        // جعل الزر يقوم بإغلاق الإضافة ليختار الفيديو براحته
        errorBtn.href = "#";
...
// دالة احتياطية سريعة بالفحص المباشر في حال تطلب الأمر
async function fallbackTabCheck() {
    const tabs = await chrome.tabs.query({ currentWindow: true, active: true });
    if (tabs && tabs[0]) checkCurrentLocationAndValidate(tabs[0]);
}
    ...
    reader.onload = function (evt) {
        try {
            ...
            chrome.storage.local.set(parsed, () => {
                ...
                chrome.runtime.sendMessage({ action: "FORCE_GET_ACTIVE_TAB" }, (response) => {
                    if (response && response.tab) checkCurrentLocationAndValidate(response.tab);
                });
                refreshAllVideosHistoryList();
            });
...
async function startLoopSequence() {
    ...
    const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true }); chrome.tabs.sendMessage(activeTab.id, { action: "START_LOOP", from, to, count }); document.getElementById('startLoopBtn').classList.add('hidden'); document.getElementById('stopLoopBtn').classList.remove('hidden');
} async function stopLoopSequence() { const [activeTab] = await chrome.tabs.query({ active: true, currentWindow: true }); chrome.tabs.sendMessage(activeTab.id, { action: "STOP_LOOP" }); document.getElementById('startLoopBtn').classList.remove('hidden'); document.getElementById('stopLoopBtn').classList.add('hidden'); }